...
//		return cfg;
	...
	//region AutoUTBook
	void visitStatement(IASTStatement stm, ICfgNode begin, ICfgNode end, ICfgNode parent) {

		ICfgNode normal = new SimpleCfgNode(stm);
		normal.setBranch(end);
		begin.setBranch(normal);
		normal.setParent(parent);
	}
	//endregion AutoUTBook
	...
	private void visitConditionStatement(IASTStatement stm, ICfgNode begin, ICfgNode end, ICfgNode _break,
									ICfgNode _continue,
								ICfgNode _throw, ICfgNode parent) {
		if (stm instanceof IASTIfStatement) {
			IASTIfStatement stmIf = (IASTIfStatement) stm;
