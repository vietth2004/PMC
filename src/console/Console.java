...
public class Console {
	...
	public Console(String functionName, String folderFullPath) throws IOException {
		String TESTING_PROJECT_PATH = folderFullPath;
		String TESTING_FUNCTIONS_LIST = "..\\Bai10_new\\local\\test.txt";
		String CONFIGURATION_FILE_PATH = "..\\Bai10_new\\local\\setting.properties";
		/**
		 * AUTHOR: THE END OF CONFIGURATION
		 */
		String [] args = new String[] { Console.LOAD_PROJECT, new File(TESTING_PROJECT_PATH).getCanonicalPath(),
...
//				Console.LOG4J_LEVEL, "debug" };
	...
	public void exportToHtml(File htmlFile, String methodName) {
		...
		float totalGenerationTime = 0;
		float totalExecutionTime = 0;

		allFunctionsTestReport += "<br/> PATH_SELECTION_STRATEGY: " + TestConfig.PATH_SELECTION_STRATEGY + "<br/>";
			...
			for (TestdataInReport testdata : item.getTestdata()) {
				...
				if (testdata.isHighlight())
					allFunctionsTestReport += "<span style=\"opacity:1\">" + testdataStatus + testdataStr + statementCov
							+ branchCov + testpath +"; generate time = " + testdata.getGenerateTime() +
							"; execution time = " + testdata.getExecutionTime() + "; generateRandomly = " +
							testdata.isGenerateRandomly() + "; isGeneratedBySDART = " + testdata.isGeneratedBySDART() +
							"<br" +
							"/></span>";
				else
					allFunctionsTestReport += "<span style=\"opacity:0.5\">" + testdataStatus + testdataStr
							+ statementCov + branchCov + testpath + "; generate time = " + testdata.getGenerateTime() +
							"; execution time = " + testdata.getExecutionTime()  + "; generateRandomly = " +
							testdata.isGenerateRandomly() + "; isGeneratedBySDART = " + testdata.isGeneratedBySDART() + "<br/></span>";

				totalGenerationTime += testdata.getGenerateTime();
				totalExecutionTime += testdata.getExecutionTime();
			}
		...
		allFunctionsTestReport += "<br/>" + summary + "<br/>" +
								...
								"; totalTime = " + (totalGenerationTime + totalExecutionTime) + "<br/>";
