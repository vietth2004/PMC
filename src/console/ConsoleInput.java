...
public class ConsoleInput {
	...
	public void findTestdata(String function) throws Exception {
		...
		functionConfig.setCharacterBound(new ParameterBound(32, 100));
		functionConfig.setIntegerBound(new ParameterBound(0, 100));
		functionConfig.setSizeOfArray(20);
		...
		//IProjectNode root = new ProjectParser(new File (TestConfig.PROJECT_PATH), null).getRootTree();

		IProjectNode root = new ProjectParser(projectFile, null).getRootTree();
	...
	private ConsoleOutput generateTestdata(IFunctionNode function) {
		...
		try {
			AbstractTestdataGeneration mars = new FastTestdataGeneration(function);
