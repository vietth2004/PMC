...
public class ReducedExternalVariableDetecter implements IExternalVariableDetecter {
    ...
    public static void main(String[] args) {
        ...
        List<INode> functionList = (List<INode>) Search
                .searchNodes(parser.getRootTree(), new FunctionNodeCondition(), "ACLCheckPasswordHash(unsigned char*,int)");

        IFunctionNode function = (IFunctionNode) functionList.get(0);
    ...
    public List<IVariableNode> findVariables() {
        ...
        for (String tempVariable : tempExternalVariables) {
            ...
            if (Search.searchInSpace(space, new VariableNodeCondition(), tempVariable).size() != 0) {
                VariableNode tempVariableNode = (VariableNode) Search
                        .searchNodes(realParent, new VariableNodeCondition(), tempVariable).get(0);
                reducedExternalVariableNodes.add(tempVariableNode);
                continue;
